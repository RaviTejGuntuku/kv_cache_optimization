...
def event_step(event: dict[str, Any]) -> int:
    lookup_step = event.get("lookup_step")
    if lookup_step is not None:
        return int(lookup_step)
...
def build_access_index(
    ...
    request_block_accesses: dict[int, list[int]] = defaultdict(list)

    for event in events:
        seq = event_step(event)
        ...
        elif event.get("event") == "request_lookup":
            for block_hash in event.get("block_hashes", []):
                request_block_accesses[int(block_hash)].append(seq)

    return node_accesses, block_accesses, request_block_accesses
...
def analyze_frontiers(events: list[dict[str, Any]], node_accesses: dict[int, list[int]]) -> list[FrontierDecision]:
    ...
    for frontier in frontiers:
        ...
        frontier_step = event_step(frontier)
        actual = next(
            (
                event
        ...
        for candidate in candidates:
            ...
            candidate_next = next_use(node_accesses, candidate_node_id, frontier_step)
        ...
        actual_next = next_use(node_accesses, actual_node_id, frontier_step)
        belady_node_id = int(belady_candidate["node_id"]) if belady_candidate else actual_node_id
        belady_next_use = (
            next_use(node_accesses, belady_node_id, frontier_step) if belady_candidate else actual_next
        )
...
def summarize_lookup_requests(events: list[dict[str, Any]]) -> dict[str, Any]:
    lookup_events = [event for event in events if event.get("event") == "request_lookup"]
    if not lookup_events:
        return {}

    total_requested_blocks = sum(int(event.get("block_count", 0)) for event in lookup_events)
    total_requested_tokens = sum(int(event.get("token_count", 0)) for event in lookup_events)
    return {
        "request_count": len(lookup_events),
        "requested_blocks": total_requested_blocks,
        "requested_tokens": total_requested_tokens,
        "mean_blocks_per_request": (
            total_requested_blocks / len(lookup_events) if lookup_events else 0.0
        ),
        "mean_tokens_per_request": (
            total_requested_tokens / len(lookup_events) if lookup_events else 0.0
        ),
    }
...
def simulate_page_cache(
    ...
    def evict_lru(cache: set[int]) -> None:
        victim = min(cache, key=lambda block: lru_last_access.get(block, -1))
        cache.remove(victim)
    ...
    for event in events:
        ...
        for block_hash in event.get("block_hashes", []):
            ...
            first_seen = block_hash not in seen_blocks
            if first_seen:
                seen_blocks.add(block_hash)
                compulsory_misses += 1

            if block_hash in lru_resident:
                lru_hits += 1
                if not first_seen:
                    lru_reuse_hits += 1
            else:
                ...
                if not first_seen:
                    lru_reuse_misses += 1
                if len(lru_resident) >= capacity:
                    evict_lru(lru_resident)
                lru_resident.add(block_hash)
            lru_last_access[block_hash] = seq
    ...
    return {
        ...
        "lru_miss_rate": (lru_misses / total_accesses) if total_accesses else 0.0,
        "belady_hits": belady_hits,
        "belady_misses": belady_misses,
        "belady_hit_rate": (belady_hits / total_accesses) if total_accesses else 0.0,
        "belady_miss_rate": (belady_misses / total_accesses) if total_accesses else 0.0,
        "compulsory_misses": compulsory_misses,
        "reuse_accesses": total_accesses - compulsory_misses,
        "lru_reuse_hits": lru_reuse_hits,
        "lru_reuse_misses": lru_reuse_misses,
        "belady_reuse_hits": belady_reuse_hits,
        "belady_reuse_misses": belady_reuse_misses,
        "lru_reuse_hit_rate": (
            lru_reuse_hits / (total_accesses - compulsory_misses)
            if total_accesses > compulsory_misses
            else 0.0
        ),
        "lru_reuse_miss_rate": (
            lru_reuse_misses / (total_accesses - compulsory_misses)
            if total_accesses > compulsory_misses
            else 0.0
        ),
        "belady_reuse_hit_rate": (
            belady_reuse_hits / (total_accesses - compulsory_misses)
            if total_accesses > compulsory_misses
            else 0.0
        ),
        "belady_reuse_miss_rate": (
            belady_reuse_misses / (total_accesses - compulsory_misses)
            if total_accesses > compulsory_misses
            else 0.0
        ),
    }
...
def main() -> None:
    ...
    node_accesses, _node_access_blocks, request_block_accesses = build_access_index(events)
    ...
    summary: dict[str, Any] = {
        ...
        "lookup_summary": summarize_lookup_requests(events),
    }

    if args.block_capacity is not None:
        summary["page_cache_simulation"] = simulate_page_cache(
            events=events,
            block_accesses=request_block_accesses,
