...
: "${SGLANG_BELADY_CACHE_FRACTION:?Set SGLANG_BELADY_CACHE_FRACTION to the reusable-cache fraction in (0,1]}"

TRACE_DIR="${TRACE_DIR:-$ROOT_DIR/artifacts/traces}"
RUN_LABEL="${RUN_LABEL:-belady_bypass_replay}"
PORT="${PORT:-30000}"
HOST="${HOST:-0.0.0.0}"
...
python -m sglang.launch_server \
  ...
  --radix-eviction-policy belady_bypass \
